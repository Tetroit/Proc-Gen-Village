...
public class RectangularModule : MonoBehaviour, IModuleGenerator
{
    ...
    public void Generate(ref ModuleInfo info, ref HouseGrid grid)
    {
        ...
        Vector2Int orientedDir = GenerationUtils.GetDirection(orientation);
        ...
        for (int i=0; i<schematicBounds.width; i++)
        {
            for (int j=0; j < schematicBounds.height; j++)
            {
                ...
                //var newItem = Instantiate(prefabs.GetPrefab("wall"), transform);
                //newItem.transform.localPosition = new Vector3(i + 0.5f, h, j + 0.5f);
                for (int k = 0; k <= h; k++)
                ...
                h++;
                roof.transform.localPosition = new Vector3(i + 0.5f, h, j + 0.5f);


                int facing = totalWidth - w - 1 < w ? 1 : -1;
                roof.transform.localRotation = Quaternion.Euler(0, (info.orientation + 3) * 90, 0);
